...
//
// `routeKey` names the entry in lib/accessControl.js, which is the single table
// mirroring the server's authorize() lists. The menu therefore shows exactly the
// screens the role can actually open — previously every staff role saw every
// item and most of them 403'd on arrival.
const menuItems = [
  { to: '/admin',            routeKey: 'dashboard',  labelKey: 'nav.dashboard',  icon: '📊', end: true },
  { to: '/admin/products',   routeKey: 'products',   labelKey: 'nav.products',   icon: '🌿' },
  { to: '/admin/orders',     routeKey: 'orders',     labelKey: 'nav.orders',     icon: '📦' },
  { to: '/admin/customers',  routeKey: 'customers',  labelKey: 'nav.customers',  icon: '👥' },
  { to: '/admin/inquiries',  routeKey: 'inquiries',  labelKey: 'nav.inquiries',  icon: '📬' },
  { to: '/admin/challans',   routeKey: 'challans',   labelKey: 'nav.challans',   icon: '📥' },
  { to: '/admin/warehouses', routeKey: 'warehouses', labelKey: 'nav.warehouses', icon: '🏬' },
  { to: '/admin/finance',    routeKey: 'finance',    labelKey: 'nav.finance',    icon: '💰' },
  { to: '/admin/farm',       routeKey: 'farm',       labelKey: 'nav.farm',       icon: '🚜' },
  { to: '/admin/users',      routeKey: 'users',      labelKey: 'nav.users',      icon: '👤' },
  { to: '/admin/audit-logs', routeKey: 'auditLogs',  labelKey: 'nav.auditLogs',  icon: '📜' },
  { to: '/admin/settings',   routeKey: 'settings',   labelKey: 'nav.settings',   icon: '⚙️' },
];
...
export default function Sidebar({ open, onClose, collapsed = false, onToggleCollapse }) {
  ...
  const visible = menuItems.filter(item => canAccess(item.routeKey, userRole));
